  ...
  try {
    ...
    if (logoInput && logoInput.files.length > 0) {
      ...
      try {
        ...
        if (oldSettings && oldSettings.logo) {
          console.log('حذف الصورة القديمة:', oldSettings.logo);
          
          try {
            await deleteFromVercelBlob(oldSettings.logo);
      ...
      // رفع اللوجو إلى Vercel Blob
      console.log('رفع اللوجو إلى Vercel Blob');
      console.log('اسم الملف الأصلي:', webpFile.name);
      console.log('امتداد الملف: webp');

      try {
        const uploadResult = await uploadToVercelBlob(webpFile, 'logo');
        logoUrl = uploadResult.url;
        console.log('تم رفع اللوجو بنجاح:', logoUrl);
    ...
    if (logoUrl) {
      ...
      try {
        ...
        if (url.hostname.includes('vercel') || url.hostname.includes('blob')) {
    ...
    if (logoPreviewElement && data.logo) {
      ...
      try {
        ...
        if (url.hostname.includes('vercel') || url.hostname.includes('blob')) {
          ...
          console.log('تم تحميل الصورة بنجاح من Vercel Blob');
        } else {
          console.warn('رابط الصورة لا يحتوي على vercel أو blob');
