...
window.publishLandingPage = async function() {
  ...
  // رفع الصور إلى Vercel Blob وجمع الروابط
  const imageUrls = [];
  try {
    if (landingPageImages.length > 0) {
      const filesToUpload = landingPageImages
        .filter(item => item && item.file)
        .map(item => item.file);
      
      if (filesToUpload.length > 0) {
        const uploadResults = await uploadMultipleToVercelBlob(filesToUpload, 'landing-pages');
...
window.viewLandingPage = async function(id) {
  try {
    ...
    // فتح صفحة الهبوط في نافذة جديدة عبر السابدومين الصحيح
    const url = DOMAIN_CONFIG.buildLandingPageUrl(subdomain, id);
    window.open(url, '_blank');
  } catch (error) {
    console.error('خطأ في عرض صفحة الهبوط:', error);
    alert('حدث خطأ أثناء فتح صفحة الهبوط');
  }
};
  ...
  try {
    ...
    try {
      ...
      if (page && page.image) {
        ...
        if (images.length > 0) {
          try {
            await deleteMultipleFromVercelBlob(images);
          } catch (err) {
...
window.updateLandingPage = async function() {
  ...
  // رفع الصور الجديدة إلى Vercel Blob
  const uploadedUrls = [];
  try {
    if (newFiles.length > 0) {
      const uploadResults = await uploadMultipleToVercelBlob(newFiles, 'landing-pages');
      uploadedUrls.push(...uploadResults.map(result => result.url));
    }
  } catch (uploadErr) {
    console.error('خطأ أثناء رفع الصور:', uploadErr);
    alert('حدث خطأ أثناء رفع الصور، حاول مجدداً');
  ...
  try {
    ...
    if (toDelete.length > 0) {
      try {
        await deleteMultipleFromVercelBlob(toDelete);
      } catch (err) {
        console.warn('تعذر حذف الصور القديمة:', err);
      }
    }
  } catch (err) {
