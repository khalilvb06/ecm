...
import { uploadToVercelBlob, deleteFromVercelBlob, uploadMultipleToVercelBlob, deleteMultipleFromVercelBlob } from './vercel-blob.js';
import { DOMAIN_CONFIG } from './domain-config.js';
  ...
  async function publishProduct() {
    ...
    if (editProductId) {
      ...
      // حذف الصور المحذوفة من Vercel Blob
      ...
      if (originalProduct && originalProduct.image) {
        try {
          ...
          if (deletedImages.length > 0) {
            try {
              await deleteMultipleFromVercelBlob(deletedImages);
            } catch (error) {
              console.log('خطأ في حذف الصور:', error);
            }
          }
        } catch (error) {
      ...
      // رفع الصور الجديدة فقط إلى Vercel Blob
      if (newImages.length > 0) {
        try {
          const uploadResults = await uploadMultipleToVercelBlob(newImages, 'products');
          existingImages.push(...uploadResults.map(result => result.url));
        } catch (error) {
          console.error('خطأ في رفع الصور الجديدة:', error);
      ...
      // إضافة منتج جديد
      // رفع الصور المتعددة إلى Vercel Blob
      const imageUrls = [];
      if (productImages.length > 0) {
        try {
          const filesToUpload = productImages.map(img => img.file);
          const uploadResults = await uploadMultipleToVercelBlob(filesToUpload, 'products');
    ...
    if (!error && data) {
      products = data.map(p => {
        ...
        try {
          ...
          console.log('خطأ في تحليل الألوان:', e, 'البيانات:', p.colors);
          colors = [];
        }
        ...
        try {
          ...
          console.log('خطأ في تحليل المقاسات:', e, 'البيانات:', p.sizes);
          sizes = [];
        }
        ...
        try {
          ...
          console.log('خطأ في تحليل العروض:', e, 'البيانات:', p.offers);
          offers = [];
        }
  ...
  function renderProductsTable() {
    ...
    products.forEach((product, idx) => {
      ...
      try {
        ...
        } else if (product.image && typeof product.image === 'string' && product.image.startsWith('[')) {
        ...
        console.log('خطأ في معالجة الصور:', error);
        ...
        <tr>
          ...
          <td>
            <div class="action-buttons-container">
              ...
              <button class="btn btn-secondary btn-sm" onclick="copyProductLink(${product.id})" 
                      title="نسخ رابط المنتج" data-bs-toggle="tooltip" data-bs-placement="top">
  ...
  async function deleteProduct(idx) {
    if (confirm('هل أنت متأكد من حذف المنتج؟')) {
      const product = products[idx];
      if (product.id) {
        // حذف الصور من Vercel Blob إذا وجدت
        if (product.image) {
          try {
            let imagesToDelete = [];
            if (product.image && typeof product.image === 'string' && product.image.startsWith('[')) {
            ...
            if (imagesToDelete.length > 0) {
              try {
                await deleteMultipleFromVercelBlob(imagesToDelete);
              } catch (error) {
                console.log('خطأ في حذف الصور:', error);
              }
            }
  ...
  function editProduct(idx) {
    ...
    if (product.colors && Array.isArray(product.colors)) {
      product.colors.forEach(c => colors.push({...c}));
    }
    
    // معالجة آمنة للمقاسات
    if (product.sizes && Array.isArray(product.sizes)) {
      product.sizes.forEach(s => sizes.push(s));
    }
    
    // معالجة آمنة للعروض
    if (product.offers && Array.isArray(product.offers)) {
      product.offers.forEach(o => offers.push({...o}));
    }
    if (product.images && product.images.length > 0) {
...
async function viewProduct(productId) {
  try {
    ...
    // فتح صفحة المنتج في المتجر في نافذة جديدة عبر السابدومين الصحيح
    const url = DOMAIN_CONFIG.buildProductUrl(subdomain, productId);
    window.open(url, '_blank');
  } catch (error) {
    console.error('خطأ في عرض المنتج:', error);
    alert('حدث خطأ أثناء فتح صفحة المنتج');
  }
}

// دالة نسخ رابط المنتج إلى الحافظة
async function copyProductLink(productId) {
  try {
    ...
    alert('تعذر نسخ الرابط، حاول لاحقاً');
  }
}
